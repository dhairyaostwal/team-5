import React from 'react'
import { AiFillStar,AiOutlineStar } from "react-icons/ai";
import { BiMessageAlt } from "react-icons/bi";
import { FaClipboardCheck } from "react-icons/fa";
import { FaAddressCard } from "react-icons/fa";
import './ViewProfile.css'

function ViewProfile() {
  return (
  <div className="container view_profile_container  m-5">
        <div className="row mt-3">
            <div className="mt-3 col-sm-12 col-md-4 text-center">
                <img src="data:image/jpeg;base64," height={300} width={250} alt="" />
                
            
            </div>
            <div className="mt-3 col-sm-12 col-md-8">
                <h2>Jeremy Rose</h2>
                <p className="field_name"> Investor</p>
                <h4 className='muted'>RATING</h4>
                <h1>8.6 <AiFillStar color='rgb(77, 195, 255)'/><AiFillStar color='rgb(77, 195, 255)'/><AiFillStar color='rgb(77, 195, 255)'/><AiFillStar color='rgb(77, 195, 255)'/><AiOutlineStar/></h1>
                <h4 className='mt-5 row-col-6'><BiMessageAlt /> Send Message <span className=' row-col-6 profile_contacts p-2'><FaClipboardCheck color='rgb(77, 195, 255);'/>Contacts</span></h4>

            
            
            
            </div>
        </div>
    <div className="row ">
        <div className="mt-4 col-sm-12 col-md-4">
        <h5 className='mt-3 text-muted'>WORK <hr /></h5>
                <h2>Spotify Newyork</h2>
                <p>170 William Street<br></br>
                New york, NY 10038-312-51</p>
        </div>
        <div className="mt-4 col-sm-12 col-md-8">
        <h3 className='mb-3'><FaAddressCard/> About</h3>
        <div className="row mb-5">
            <div className="col-4">Phone: </div>
            <div className="col-8">+919658458755</div>
            <div className="col-4">Address: </div>
            <div className="col-8">Newyork City</div>
            <div className="col-4">E-Mail: </div>
            <div className="col-8">hasan@gmail.com</div>
        </div>
        </div>
    </div>
  </div>
  )
}

export default ViewProfile;